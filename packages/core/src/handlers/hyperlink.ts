...
class HyperlinkHandler {
  ...
  showHyperlinkViewButton(link: string | ''): void {
    ...
    const hyperLinkText = document.getElementById(
      strings.VIEW_HYPERLINK_ANCHOR_ID
    ) as HTMLSpanElement;

    if (viewHyperlinkContainer && hyperLinkText) {
      viewHyperlinkContainer.style.display = 'block';

      if (!this.hyperlinkAlreadyPositioned) {
        const selection = window.getSelection();
        if (selection) {
          const range = selection.getRangeAt(0);
          const rect = range.getBoundingClientRect();
          ...
          this.hyperlinkAlreadyPositioned = true;
        }
      }
